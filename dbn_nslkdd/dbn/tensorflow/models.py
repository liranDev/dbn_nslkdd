    ...
    def _initialize_weights(self, weights):
        if weights:
            ...
            if self.activation_function == 'sigmoid':
                ...
                self.W = weight_variable(tf.random.truncated_normal, [self.n_hidden_units, self.n_visible_units],
                                         stddev)
...
class SupervisedDBNClassification(TensorFlowAbstractSupervisedDBN, ClassifierMixin):
    ...
    def _build_model(self, weights=None):
        ...
        self.cost_function = tf.reduce_mean(
            tf.nn.softmax_cross_entropy_with_logits_v2(logits=self.y, labels=tf.stop_gradient(self.y_)))
